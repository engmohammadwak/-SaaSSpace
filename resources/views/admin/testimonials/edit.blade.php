...
<head>
    ...
    <title>Edit Testimonial – SaaSSpace Admin</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .current-avatar { width: 56px; height: 56px; border-radius: 50%; object-fit: cover; }
            ...
            <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="client_title" value="{{ old('client_title', $testimonial->client_title) }}" required>
                    </div>
                    <div class="form-group full">
                        <label>Review Content *</label>
                        <textarea name="content">{{ old('content', $testimonial->content) }}</textarea>
                    </div>
                    <div class="form-group">
                        <label>Avatar (leave empty to keep current)</label>
                        @if($testimonial->client_avatar)
                        <img src="{{ Storage::url($testimonial->client_avatar) }}" alt="current" class="current-avatar">
                        <p class="current-label">Current avatar ↑</p>
                        @endif
                        <input type="file" name="client_avatar" accept="image/*">
                    </div>
                    <div class="form-group">
                        <label>Rating</label>
                        <select name="rating">
                            @for($i = 5; $i >= 1; $i--)
                            <option value="{{ $i }}" {{ $testimonial->rating == $i ? 'selected' : '' }}>{{ $i }} Stars {{ str_repeat('★', $i) }}</option>
                            @endfor
                        </select>
                    </div>
                    <div class="form-group">
                        <div class="toggle-row">
                            <input type="checkbox" name="is_active" id="is_active" {{ $testimonial->is_active ? 'checked' : '' }}>
